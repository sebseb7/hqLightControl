    ...
    putCeiling: (n, r=0, g=0, b=0, w=0) =>
        fmt = (c) ->
            ...
            while s.length < 2
                s = "0#{s}"
            s
        @send_cmd "02F#{n+0}#{fmt r}#{fmt g}#{fmt b}#{fmt w}"

    setPixel: (x=0,y=0, r=0, g=0, b=0) =>
        fmt = (c) ->
            s = Math.max(0, Math.min(255, Math.floor(c))).toString 16
            while s.length < 2
                s = "0#{s}"
            s
        @send_cmd "02#{fmt x}#{fmt y}#{fmt r}#{fmt g}#{fmt b}"

    flush: =>
        if @sock
            if /^CeilingLED/.test(@name)
                for i in [0..Math.min(@ceiling.length-1, 3)]
                    if @ceiling[i]
                        console.log "ceil", "02F#{i+1}#{@ceiling[i]}"
                        @send_cmd "02F#{i+0}#{@ceiling[i]}"
